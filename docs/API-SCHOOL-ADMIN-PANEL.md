...
- **Просмотр** опубликованного в каталоге курса: полные секции, уроки (включая черновики), блоки контента (`content`, `fileUrl`, …) и тест с правильными ответами — см. **`API-SCHOOL-ADMIN-BACKEND.md`** (раздел про просмотр материалов).
